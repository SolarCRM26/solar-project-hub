...
export const ChecklistRunner = ({ taskId, projectId }: ChecklistRunnerProps) => {
  const { user } = useAuth();
  ...
  // Load active run
  const activeRun = existingRuns.find((run: any) => run.status === 'in_progress');
  ...
  const completedRequiredItems = items.filter(item => item.required && item.checked).length;
        ...
        <div className="space-y-4">
          ...
          {!activeRun && (
            <div className="space-y-2">
              <label className="text-sm font-medium">Select Checklist Template</label>
            ...
            <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm">
              <div className="flex items-start gap-2">
                ...
                <p className="text-amber-900 dark:text-amber-100">
                  Complete all <strong>{requiredItems}</strong> required items to finish this checklist.
                  {completedRequiredItems > 0 && ` (${completedRequiredItems}/${requiredItems} done)`}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg p-3 text-sm">
              <div className="flex items-start gap-2">
                ...
                <p className="text-green-900 dark:text-green-100">
                  Checklist completed on {new Date(activeRun.completed_at).toLocaleString()}
                </p>
              </div>
            </div>
          )}
