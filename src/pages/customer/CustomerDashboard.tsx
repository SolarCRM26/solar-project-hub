...
import { Eye, FolderKanban, FileText, CheckCircle, Package } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <div>
        ...
        <p className="text-muted-foreground mt-1">View your solar project progress</p>
      </div>
      ...
      {projects.length === 0 ? (
        <Card className="border-border/50">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">Your projects will appear here once assigned.</p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {project.stage === 'closeout_delivered' && (
                      ...
                      >
                        ...
                        {generating ? 'Generating...' : 'Download Closeout Package'}
                      </Button>
                    )}
                  </div>
                </div>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{project.capacity_kw ? `${project.capacity_kw} kW` : '—'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{project.target_completion || '—'}</p>
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      {projectMilestones.map(m => (
                        <div key={m.id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/50">
                          <div className={`w-3 h-3 rounded-full ${m.completed_at ? 'bg-success' : 'bg-muted-foreground/30'}`} />
                          <span className={`text-sm ${m.completed_at ? 'line-through text-muted-foreground' : ''}`}>{m.name}</span>
                          {m.due_date && <span className="text-xs text-muted-foreground ml-auto font-mono">{m.due_date}</span>}
                        </div>
                      ))}
                    </div>
                  </div>
