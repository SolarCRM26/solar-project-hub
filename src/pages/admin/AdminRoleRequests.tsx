...
const AdminRoleRequests = () => {
  ...
  const approveRequest = useMutation({
    mutationFn: async (requestId: string) => {
      const { data, error } = await (supabase as any).rpc('approve_role_request', {
  ...
  const rejectRequest = useMutation({
    mutationFn: async ({ requestId, reason }: { requestId: string; reason: string }) => {
      const { data, error } = await (supabase as any).rpc('reject_role_request', {
        p_request_id: requestId,
        p_admin_id: user?.id,
  ...
  const getRoleBadge = (role: string) => {
    const colors: Record<string, string> = {
      ...
      engineer: 'bg-orange-100 text-orange-800 border-orange-300',
      customer: 'bg-green-100 text-green-800 border-green-300',
      project_manager: 'bg-purple-100 text-purple-800 border-purple-300',
      qa_manager: 'bg-pink-100 text-pink-800 border-pink-300',
    };
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Approve or reject user role requests</p>
      </div>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Are you sure you want to approve the {request.requested_role} role for {profile?.full_name || 'this user'}?
                                  This will grant them immediate access.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                          ...
                          <AlertDialog onOpenChange={(open) => {
                            ...
                            else { setSelectedRequest(null); setRejectionReason(''); }
                          }}>
                            <AlertDialogTrigger asChild>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                <AlertDialogAction
                                  onClick={() => rejectRequest.mutate({ requestId: request.id, reason: rejectionReason })}
